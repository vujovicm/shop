...
function App() {
  ...
  const dropdownOptions = [
    ...
    },
    {
      ...
      sortMethod: (a, b) => calculatePrice(a) - calculatePrice(b),
    },
    {
      id: 4,
      label: "Sort by price desc",
      sortMethod: (a, b) => calculatePrice(b) - calculatePrice(a),
    },
  ];
  ...
  const sortMethod = selectedSortOption
    ? selectedSortOption.sortMethod
    : defaultSort;
            ...
            <div key={product.id} className="card">
              ...
              {product.sale ? (
                <div className="priceDiv">
                  <p className="striked"> {product.price} rsd </p>{" "}
                  <p> {calculatePrice(product)} rsd </p>
                </div>
              ) : (
                <div> {product.price} rsd </div>
              )}
              {product.sale && (
                <div className="onSale"> {product.sale}% OFF </div>
              )}
            </div>
          );
        })}
